...
function resize() {
  ...
  const host = canvas?.closest('.oceanus-hero')
  if (!canvas || !host || !ctx)
    return
...
<template>
  <canvas
    ref="canvasRef"
    class="oceanus-hero-particles"
    aria-hidden="true"
  />
</template>

<style scoped>
.oceanus-hero-particles {
  position: absolute;
  inset: 0;
  z-index: 2;
  width: 100%;
  ...
  pointer-events: none;
  opacity: 0;
  animation: oceanus-hero-particles-in 0.5s ease-out 0.2s forwards;
}

@keyframes oceanus-hero-particles-in {
  to {
    opacity: 1;
  }
}
