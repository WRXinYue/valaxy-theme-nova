...
<template>
  <section
    v-if="fm?.getStarted"
    ref="sectionRef"
    class="get-started-section nova-home-below-hero relative overflow-hidden"
    p="t-260px b-220px"
    flex="~ col center"
    :class="{ 'nova-home-below-hero--in-view': isRevealed }"
    style="--nova-home-section-i: 3"
  >
    <div class="framework-row">
      <div
        v-for="(card, i) in fm.getStarted?.cards"
        :key="i"
        class="nova-home-card-conic-shell framework-card-shell"
        style="--nova-home-card-br: 16px"
      >
        <div
...
.nova-glass-cta {
  ...
  &::before {
    ...
    animation: nova-glass-cta-border-flow 3s linear infinite;
    opacity: 0.55;
...
.nova-glass-cta__shimmer {
  ...
  animation: nova-glass-cta-shimmer 3s linear infinite;
  pointer-events: none;
  z-index: 0;
}

.nova-glass-cta:hover .nova-glass-cta__shimmer {
  animation-duration: 1.5s;
}

.nova-glass-cta__label {
  position: relative;
  z-index: 3;
}

@keyframes nova-glass-cta-border-flow {
  0% {
...
@keyframes nova-glass-cta-shimmer {
  0% {
