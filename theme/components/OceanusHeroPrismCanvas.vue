...
import { PRISM_FRAGMENT_SHADER, PRISM_VERTEX_SHADER } from './oceanus-hero-prism-shaders'
...
function createShader(glCtx: WebGLRenderingContext, type: number, source: string) {
  ...
  if (!glCtx.getShaderParameter(shader, glCtx.COMPILE_STATUS)) {
    console.error('[OceanusHeroPrism]', glCtx.getShaderInfoLog(shader))
...
function createProgram(glCtx: WebGLRenderingContext, vs: WebGLShader, fs: WebGLShader) {
  ...
  if (!glCtx.getProgramParameter(prog, glCtx.LINK_STATUS)) {
    console.error('[OceanusHeroPrism]', glCtx.getProgramInfoLog(prog))
...
onMounted(() => {
  ...
  hostEl = root.closest('.oceanus-hero')
  if (!hostEl)
    return
  ...
  if (!gl) {
    console.warn('[OceanusHeroPrism] WebGL not available')
    return
  }
...
<template>
  <div ref="rootRef" class="oceanus-hero-prism">
    <div class="oceanus-hero-prism__fallback" aria-hidden="true" />
    <canvas
      ref="canvasRef"
      class="oceanus-hero-prism__canvas"
      :class="{ 'oceanus-hero-prism__canvas--ready': webglReady }"
      aria-hidden="true"
    />
  </div>
</template>
